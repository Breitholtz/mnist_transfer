...
def get_learning_rate(task,architecture):
    ...
    if architecture=="lenet":
        ...
        lr=0.0003 ## this should correspond to resnet or some other large architecture
    return lr
        
    
#### TODO: make generator work with the training    
def train_posterior(alpha,generator=None, val_generator=None, x_train=[],y_train=[],prior_weights=None,x_test=[],y_test=[],save=True,task=2,binary=False,batch_size=128,architecture="lenet",seed=69105,image_size=32):
        ...
        ## choose loss function, optimiser etc. @TODO: make momentum etc be passed in
        ...
        ###########################################################################
        # train for X epochs         used to be: until we reach \epsilon error on the whole of S
        ###########################################################################
...
#                 res = pickle.load(rfp)
#                 if seed in res["seed"].values:
#                     # remove old and replace with new results
#                     res=res[res.seed.isin([seed]) == False]
